...
const getMaxChromaIndexed = makeEdgeSeekerIndexed((r, g, b) => p3ToOklch(r, g, b));
...
export function edgeSeekerIndexed (oklch, out, checkInGamut = false) {
	const l = oklch[0], c = oklch[1], h = oklch[2];
	if (l <= 0) {
		out[0] = out[1] = out[2] = 0;
		return out;
	...
	const maxChroma = getMaxChromaIndexed(l, h);
	return oklchToClippedP3(l, c > maxChroma ? maxChroma : c, h, out);
}
